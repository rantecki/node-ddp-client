...
var DDPClient = function(opts) {
  ...
  self.url  = opts.url;
  // arguments for heartbeat/initial reconnection control
  self.heartbeatInterval = ("heartbeatInterval" in opts) ? opts.heartbeatInterval : 10000;
  self.heartbeatTimeout = ("heartbeatTimeout" in opts) ? opts.heartbeatTimeout : 5000;
  self.initialReconnectTimeout = ("initialReconnectTimeout" in opts) ? opts.initialReconnectTimeout : 5000;
...
DDPClient.prototype._onHeartbeatTimer = function() {
  var self = this;

  if(self._isConnecting || self._connectionFailed || self._isClosing)
    return;

  self._send({ msg : "ping" });
  self._heartbeatTimeoutTimer = setTimeout(self._onheartbeatTimeoutTimer.bind(self), self.heartbeatTimeout);
}

DDPClient.prototype._onheartbeatTimeoutTimer = function() {
  var self = this;

  if(self._isConnecting || self._connectionFailed || self._isClosing)
    return;

  self.emit("heartbeat-failed");
}
...
DDPClient.prototype._message = function(data) {
  ...
  if (!data.msg) {
    ...
    // If DDP Version is > "pre1", start sending pings
    if (self.ddpVersion !== "pre1") {
      if(self._heartbeatTimer)
        clearTimeout(self._heartbeatTimer);

      self._heartbeatTimer = setTimeout(self._onHeartbeatTimer.bind(self), self.heartbeatInterval);
    }
  ...
  // minimal heartbeat response for ddp pre2
    ...
    if(self._heartbeatTimeoutTimer)
      clearTimeout(self._heartbeatTimeoutTimer);
    if(self._heartbeatTimer)
      clearTimeout(self._heartbeatTimer);
    self._heartbeatTimer = setTimeout(self._onHeartbeatTimer.bind(self), self.heartbeatInterval);
  }
